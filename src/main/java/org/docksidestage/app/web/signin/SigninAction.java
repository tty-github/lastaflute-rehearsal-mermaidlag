...
import org.docksidestage.app.web.base.MermaidlagBaseAction;
 ...
 * @author jflute
 */
public class SigninAction extends MermaidlagBaseAction {
    ...
    public HtmlResponse signin(SigninForm form) {
        ...
        return mermaidlagLoginAssist.loginRedirect(form.account, form.password, op -> op.rememberMe(form.rememberMe), () -> {
            return redirect(MypageAction.class);
        });
    }

    private void moreValidate(SigninForm form, MermaidlagMessages messages) {
        if (isNotEmpty(form.account) && isNotEmpty(form.password)) {
            if (!mermaidlagLoginAssist.checkUserLoginable(form.account, form.password)) {
