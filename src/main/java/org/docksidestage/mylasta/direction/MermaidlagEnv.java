 ...
 * @author FreeGen
 */
public interface MermaidlagEnv {
    ...
    /** The key of the configuration. e.g. /tmp/lastaflute/mermaidlag */
    ...
    /** The key of the configuration. e.g. mermaidlag-support@annie.example.com */
     ...
     * The value is, e.g. /tmp/lastaflute/mermaidlag <br>
     ...
     * The value is, e.g. mermaidlag-support@annie.example.com <br>
     * comment: Mail Address for Mermaidlag Support
     * @return The value of found property. (NotNull: if not found, exception but basically no way)
     */
     ...
     * @author FreeGen
     */
    public static class SimpleImpl extends ObjectiveConfig implements MermaidlagEnv {

        /** The serial version UID for object serialization. (Default) */
        private static final long serialVersionUID = 1L;

        public String getLastaDiSmartDeployMode() {
            return get(MermaidlagEnv.lasta_di_SMART_DEPLOY_MODE);
        }

        public String getDevelopmentHere() {
            return get(MermaidlagEnv.DEVELOPMENT_HERE);
        }

        public boolean isDevelopmentHere() {
            return is(MermaidlagEnv.DEVELOPMENT_HERE);
        }

        public String getEnvironmentTitle() {
            return get(MermaidlagEnv.ENVIRONMENT_TITLE);
        }

        public String getFrameworkDebug() {
            return get(MermaidlagEnv.FRAMEWORK_DEBUG);
        }

        public boolean isFrameworkDebug() {
            return is(MermaidlagEnv.FRAMEWORK_DEBUG);
        }

        public String getTimeAdjustTimeMillis() {
            return get(MermaidlagEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public Long getTimeAdjustTimeMillisAsLong() {
            return getAsLong(MermaidlagEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public String getLogLevel() {
            return get(MermaidlagEnv.LOG_LEVEL);
        }

        public String getLogConsoleLevel() {
            return get(MermaidlagEnv.LOG_CONSOLE_LEVEL);
        }

        public String getLogFileBasedir() {
            return get(MermaidlagEnv.LOG_FILE_BASEDIR);
        }

        public String getMailSendMock() {
            return get(MermaidlagEnv.MAIL_SEND_MOCK);
        }

        public boolean isMailSendMock() {
            return is(MermaidlagEnv.MAIL_SEND_MOCK);
        }

        public String getMailSmtpServerMainHostAndPort() {
            return get(MermaidlagEnv.MAIL_SMTP_SERVER_MAIN_HOST_AND_PORT);
        }

        public String getMailSubjectTestPrefix() {
            return get(MermaidlagEnv.MAIL_SUBJECT_TEST_PREFIX);
        }

        public String getMailReturnPath() {
            return get(MermaidlagEnv.MAIL_RETURN_PATH);
        }

        public String getMailAddressSupport() {
            return get(MermaidlagEnv.MAIL_ADDRESS_SUPPORT);
        }

        public String getJdbcDriver() {
            return get(MermaidlagEnv.JDBC_DRIVER);
        }

        public String getJdbcUrl() {
            return get(MermaidlagEnv.JDBC_URL);
        }

        public String getJdbcUser() {
            return get(MermaidlagEnv.JDBC_USER);
        }

        public String getJdbcPassword() {
            return get(MermaidlagEnv.JDBC_PASSWORD);
        }

        public String getJdbcConnectionPoolingSize() {
            return get(MermaidlagEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public Integer getJdbcConnectionPoolingSizeAsInteger() {
            return getAsInteger(MermaidlagEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public String getServerDomain() {
            return get(MermaidlagEnv.SERVER_DOMAIN);
        }
    }
}
